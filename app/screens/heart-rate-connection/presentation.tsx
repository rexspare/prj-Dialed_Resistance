...
export const Presentation = observer(function WelcomeScreen() {
  ...
  const start = () => {
    const metricUpdateRate = variables?.find((x) => x?.title == 'Metric update rate (ms)')
    const UPDATE_INTERVAL = metricUpdateRate?.value ?
      Number(metricUpdateRate?.value) < 500 ? 500
        : Number(metricUpdateRate?.value) : 1000
    const interval = setInterval(getData, UPDATE_INTERVAL)
